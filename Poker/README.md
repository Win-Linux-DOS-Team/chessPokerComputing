...
该代码支持命令行启动，在 Windows 操作系统下应当使用 Visual Studio 以 ISO C++17 或以上的 C++ 标准、/W4 的警告等级以及多线程（调试）的运行库对所有 ``Poker/Poker.vcxproj`` 中声明的所有配置和所有平台进行批生成，在其它操作系统下可使用 ``g++ Poker.cpp -Wall -o Poker`` 直接生成，要求编译器最低支持 C++11 标准。

在以上生成中，Visual Studio 对文件的代码分析以及 g++ 的输出应当**不含有任何错误、警告或消息**。
