...
```
protected:
	std::mt19937 seed = std::mt19937(std::random_device{}()); // 用于存储随机数种子
	std::string name = "扑克牌"; // 用于存储扑克游戏的名称
	Values values{}; // 用于存储大小
	std::vector<std::vector<Card>> players{}; // 用于存储玩家手上的扑克牌
	std::vector<Card> deck{}; // 用于存储地主牌或牌堆
	std::vector<std::vector<Hand>> records{}; // 用于存储游戏记录
	Player currentPlayer = INVALID_PLAYER, dealer = INVALID_PLAYER; // 分别用于指示当前需要进行操作和首轮第一位出牌的玩家
	Hand lastHand{}; // 用于存储最后一手非空 hand
	std::vector<Amount> amounts{}; // 用于存储结算信息
	Status status = Status::Ready; // 用于存储状态
```

基类中主要的成员函数如下，它们的返回值类型均为 bool，``status`` 仅能在这些函数中被更改；在基类和派生类中，仅这些成员函数是公共的。

1) 构造函数：初始化随机数种子，派生类的构造函数中应当调用该过程并声明扑克游戏的名称；该过程仅在实例创建时被调用；调用成功后，状态将变更为缺省值 Ready。
...
5) getCurrentPlayer：获取当前玩家；由于该过程是只读的，故不会发生状态变更。
6) setLandlord：以“叫地主—抢地主”或叫分的方式设置地主信息；该过程仅允许在 Dealt 状态下涉及地主信息的扑克游戏中进行；地主信息被完整地写入 records[0] 后，状态将变更为 Assigned。
7) start：开牌，该过程将依照 hand 读写 records，部分扑克游戏对开牌的 hand 有特定要求；该过程仅允许在 Assigned 状态下进行；调用成功后，状态将变更为 Started 或 Over（开局直接结束游戏）。
8) play：出牌，该过程将依照 hand 读写 records；该过程仅允许在 Started 状态下进行；游戏结束后，状态将变更为 Over。
9) set：导入牌局，该过程将先尝试将参数作为文件路径进行处理，处理失败则将参数作为数据进行处理；该过程仅允许不低于 Initialized 的状态下被调用；调用成功后，状态将根据导入牌局的情况进行自适应变更。
10) display：显示牌局；该过程允许在任何状态下被调用；由于该过程是只读的，故不会发生状态变更。

基类中需要被（部分）派生类重写的子函数如下（如有需要请参阅本文档中确定性规则中的通用概念）：

/* Poker::initialize */

| 序号 | 子方法函数名 | 重写情况 | 需要重写该函数的派生类 | 用途 |
| - | - | - | - | - |
| 1 | checkPlayerCount | 纯虚函数 | 所有派生类 | 判断玩家数量是否满足所属扑克游戏要求 |

/* Poker::setLandlord */

| 序号 | 子方法函数名 | 重写情况 | 需要重写该函数的派生类 | 用途 |
| - | - | - | - | - |
| 1 | nextPlayer | 部分重写 | 三两一、五瓜皮、七鬼五二三和七鬼五二一 | 获取下一位当前玩家 |

/* Poker::start */

| 序号 | 子方法函数名 | 重写情况 | 需要重写该函数的派生类 | 用途 | 调用的其它子方法函数 |
| - | - | - | - | - | - |
| 1 | checkStarting | 部分重写 | 斗地主和四人斗地主 | 大部分扑克游戏要求第一回合的引牌必须带有所有玩家手上的扑克牌中价值最小的牌 | -- |
| 2 | isRealHand | 纯虚函数 | 所有派生类 | 判断一手牌是否为实际出牌 | -- |
| 3 | coverLastHand | 纯虚函数 | 所有派生类 | 判断当前一手牌能否盖过上一手牌 | 此子函数会调用 ``isRealHand`` 函数 |
| 4 | processHand | 纯虚函数 | 所有派生类 | 将传递的 hand 参数中的牌组升变为一手牌 | 此子函数会调用 ``coverLastHand`` 函数 |
| 5 | isAbsolutelyLargest | 纯虚函数 | 所有派生类 | 判断该手牌的价值是否无条件最大 | -- |
| 6 | processBasis | 部分重写 | 斗地主、四人斗地主、五瓜皮、七鬼五二三和七鬼五二一 | 实时显示倍数或积分信息 | 此子函数会调用 ``isAbsolutelyLargest`` 函数 |
| 7 | isOver | 部分重写 | 五瓜皮、七鬼五二三和七鬼五二一重写 | 大部分扑克游戏在第一位玩家出完手上所有的扑克牌时结束而少部分扑克游戏要求牌堆为空且只剩一位玩家手上拥有扑克牌时结束 | -- |
| 8 | computeAmounts | 部分重写 | 斗地主、四人斗地主、锄大地和三两一 | 游戏结束时计算结算信息 |

/* Poker::display */

| 序号 | 子方法函数名 | 重写情况 | 需要重写该函数的派生类 | 用途 |
| - | - | - | - | - |
| 1 | getBasisString | 部分重写 | 斗地主、四人斗地主、五瓜皮、七鬼五二三和七鬼五二一 | 实时显示倍数或积分信息 |
| 2 | getPreRoundString | 纯虚函数 | 所有派生类 | 显示预备回合信息 |
| 3 | getAmountString | 部分重写 | 五瓜皮、七鬼五二三和七鬼五二一 | 通用地显示结算信息或在结算信息处同时显示先出完牌的排名、积分和积分排名 |

一般地，除了函数 ``processHand`` 会对 ``hand`` 进行排序并主动清空或修改 ``possibleHands`` 以供二义性牌组作进一步确认外，所有 bool 型函数的返回值为 ``false`` 时，不会对传递为引用的任何参数做任何更改。

符号排序：

1) 代码块排序：头文件 -> 宏（不包含用于类或函数内平台控制的宏） -> 命名空间（本程序未声明任何命名空间）-> 简易类型定义 -> constexpr -> 枚举类 -> 结构体（成员变量 -> 成员函数）-> 类（公有成员变量 -> 受保护成员变量 -> 私有成员变量 -> 私有成员函数 -> 受保护成员函数 -> 公有成员函数）-> 非 main 函数 -> main 函数，其中，在结构体的成员函数和类的公有成员函数中，将所有带有 ``operator`` 关键字的函数放置末尾，并依照“先重载运算符的函数后强制转型函数”的顺序排序。
2) 符号绑定：同一个结构体或类中的所有构造函数和析构函数视为同一符号参与规则 3) 进行进一步排序，在此整体内依照“先构造函数后析构函数，以参数数量从少到多的规则排列构造函数”的方式进行排序，由于所有构造函数和析构函数均属于公有成员函数，因此此排序不与规则 1) 冲突；重载函数视为同一符号参与规则 3) 进行进一步排序，该整体内的顺序遵从规则 3)；在规则 3) 中从前往后扫描符号时，扫描完基类的构造函数和析构函数后应立即扫描派生类（及派生类的派生类）的构造函数和析构函数，若派生类（及派生类的派生类）重写了基类的某一虚函数（或其重载），则先从前往后扫描基类中的虚函数及其所有重载，再从前往后依次扫描所有重写了该虚函数及其重载的函数。
3) 变量排序：将 main 函数作为最初的当前符号，从前往后扫描当前符号，在当前符号中每出现新符号时依照代码块排序将其放置在其所属代码块中最后的位置，如新符号与当前符号属于同一代码块，则放置在代码块内当前符号前的最后位置，随后将新符号作为当前符号进行扫描，当前符号解析完成后，返回原有当前符号中未扫描部分的头部位置继续完成扫描，直至 main 函数的解析工作结束，以此以递归的方式完成排序；同一函数（及其子语句）中的局部变量、同一枚举型中有序的枚举和 switch 语句中的 case 标签，可以不遵从此规则。

编程风格（依照每点风格在代码中的位置从前往后排序）：

1) 关于 const：不在类型声明时使用 const 修饰符，不在函数签名的类型前使用 const 修饰符（违反 C5266 警告），除此之外，对所有不希望在初始化后被修改的变量和不希望对成员变量做出修改的成员函数均应当使用 const 修饰符；能使用 constexpr 的非系统宏则使用 constexpr。
2) 缺省值：所有变量在声明时均必须指派初始值；请注意，``nullptr`` 也属于初始值。
3) 代码取舍：即使可能会十分轻微地牺牲一些性能，仍以函数为最小安全单元做必要的安全检验（例如防止恶意的内存修改导致状态不低于 Dealt 时 ``players`` 为空）；如遇使用更多代码可以换取一些性能而使用精简代码可能会牺牲一些性能的情况，允许并在性能和精简代码之间进行平衡取舍。
4) 最小权限原则：对函数和变量在满足设计需求范围的前提下使用最受限的修饰符，例如，使用 const（位于函数签名后）修饰不对成员变量做出修改的成员函数，使用 const 声明局部变量常量，使用 private 声明私有函数。
5) 虚函数：虚函数在基类中必须使用 virtual 修饰符，在派生类及派生的派生类中均不使用 virtual 修饰符，重写时必须使用 override 修饰符，作为最后实现时必须带有 final 修饰符。

缩进使用风格：

1) 统一使用制表符进行缩进，缩进应当正确反映代码的层次结构；理论上，非字符串的代码中不应含有两个或两个以上的连续的空格，因此，可通过搜索非字符串中的连续的两个空格以完成检查。
2) 若一行结束时该行中存在未闭合的左括号，则该行被记为开放行，含有开放行中最后一个未闭合的左括号对应的右括号的行则被记为闭合行，随后，此处规定，开放行与闭合行之间的行的缩进量均需比开放行和闭合行的缩进量多一，开放行中开放的括号数量应当与闭合行中闭合开放行中开放的括号数量相等，必要时可在自开放行至闭合行前一行的行末处添加对齐的反斜杠符号，反斜杠符号前使用制表符进行填充（通常以行末位于最右侧的行填充一至二个制表符作为基准），但不得删除原本存在于行末的空格字符；通常，开放的括号位于开放行的行末；此规则在嵌套式相应地进行递归执行。
3) 因代码块切换或出于注释需要时，可以出现相应的无代码行，但此类无代码行应当遵循上下文进行缩进；由于 Visual Studio 会将不含有代码的行自动清空为不含有任何字符的空行，因此切忌将包含此类无代码行的代码在 Visual Studio 编辑界面中进行粘贴。

花括号使用风格：

1) 用于对象初始化或给变量赋值的花括号通常写于同一行，如此类花括号中的内容较多，允许将此类花括号中的左右花括号分别置于开放行的行末和闭合行的行首，随后依照缩进风格书写代码。
2) 对于用于代码结构（命名空间、枚举类、结构体、类、非 lambda 函数和所有控制语句）的花括号，为便于检查配对，统一将此类左花括号独占代码结构签名的下一行并使其缩进量与代码结构签名的缩进量相等，右花括号独占代码结构内容的下一行并与代码结构签名对齐。
3) 允许使用初始化列表且内容为空的构造函数、lambda 函数以及函数体中有效代码行数不超过 1 行的虚函数以一行的形式呈现，若花括号内无有效代码，则此对花括号之间不应该含有任何字符，否则两侧的花括号需分别与其内容之间保留一个空格。
4) 在 ``g++ -Wall -Wextra -pedantic`` 不报错误或警告且省略花括号不会变更代码语义的前提下，能不使用花括号则不使用花括号。
...
4) 一副扑克牌（Poker）：一副完整的扑克牌包含 54 张扑克牌，即包含大小王（双王）以及由四种花色与 13 个点数组合而成的 $4 \times 13 = 52$ 张不重复的牌，一副不含大小王的扑克牌则由一副完整的扑克牌去除大小王而来，仅包含由四种花色与 13 个点数组合而成的 $4 \times 13 = 52$ 张不重复的牌；检查扑克牌完整性时，通常基于花色分类后清点点数以确认每张扑克牌都存在且唯一，并单独确认大小王是否唯一；如扑克游戏不需要使用大小王，人们会取一副完整的扑克牌，将大小王放置在一旁以使用一副不含大小王的扑克牌，待扑克游戏结束后，将使用的 52 张扑克牌连同大小王一起放回扑克牌牌盒中。
5) 一副带封面牌和广告牌的扑克牌：除包含一副完整的扑克牌外，一副带封面牌和广告牌的扑克牌还包含一张指示生产厂家信息的封面牌以及若干张广告牌（现实中绝大多数成副的扑克牌不含广告牌），其中，封面牌在日常中常被用于替补某张不慎遗失或损毁的扑克牌；通常，正规的扑克牌厂商只会印制一副完整的扑克牌（或最多在此基础上增添一张封面牌）而不会印制一副不带有大小王的扑克牌，并由配套的扑克牌牌盒进行保护；请注意，一些厂商出于宣传目的（如消防宣传、预防艾滋病和月饼促销）会把宣传内容印在普通扑克牌（含大小王）的牌面上，只要牌面清晰地指示了牌面值，这些牌就不属于定义上的封面牌或广告牌。
...
4) 一副扑克牌：通过两层循环遍历四种花色与 13 个点数的所有组合构建一副不含大小王的扑克牌，在此基础上追加大小王则可构建一副完整的扑克牌；构建完成后，以 ``vector<Card>`` 的形式进行保存。
5) 一副带封面牌和广告牌的扑克牌：未在本目录中使用过该定义。
...
程序在识别本扑克游戏时，会先捕获其英文，如果英文未能与本存储库所支持的任何扑克游戏匹配，则将所有数字相应地替换为中文再进行识别；以下为本存储库所支持的扑克游戏及其确定性规则和实现说明，其中所述的“牌”进一步指代有牌面值的扑克牌。

#### 斗地主

此扑克游戏使用一副完整的扑克牌，每局玩家人数为 3 人，牌局初始时，程序向每位玩家随机派发 17 张牌，并保留 3 张牌作为底牌（地主牌）。在日常生活中，玩家应当绕圈而坐，面朝内，发牌者将一副完整的扑克牌以牌面背对玩家的形式地进行随机打乱，并保持牌面背对玩家的形式逐张进行派发（通常按逆时针顺序发牌和出牌），直到牌堆剩余三张牌，该三张牌在地主确定之前保持不可见。
...
| 4 | 三条 | ``Type::Triple`` | 三张点数相同的牌 | 依照花色降序 |
...
除大小比较变为 7＞大王＞小王＞5＞2＞3＞A＞K＞Q＞J＞10＞9＞8＞6＞4 之外，其余事宜与七鬼五二三一致。
