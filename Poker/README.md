...
该代码支持命令行启动，在 Windows 操作系统下应当使用 Visual Studio 以 ISO C++17 或以上的 C++ 标准、/W4 的警告等级以及多线程（调试）的运行库对所有 ``Poker/Poker.vcxproj`` 中声明的所有配置和所有平台进行批生成，在其它操作系统下可使用 ``g++ Poker.cpp -Wall -Wextra -o Poker`` 直接生成，要求所使用的 g++ 最低支持 C++11 标准。

在以上生成中，Visual Studio 对文件的代码分析以及未指定或指定了任一 C++ 11 或更高 C++ 标准的上述 g++ 命令的输出应当**不含有任何错误、警告或消息**。
...
1) 代码块排序：头文件 -> 宏（不包含用于类或函数内平台控制的宏） -> 简易类型定义 -> constexpr -> 命名空间 -> 枚举类 -> 结构体（成员变量 -> 成员函数）-> 类（私有成员变量 -> 受保护成员变量 -> 公有成员变量 -> 私有成员函数 -> 受保护成员函数 -> 公有成员函数）-> 非 main 函数 -> main 函数，其中，在结构体的成员函数和类的公有成员函数中，将所有带有 ``operator`` 关键字的函数放置末尾，并依照“先重载运算符的函数后强制转型函数”的顺序排序。
...
1) 关于 const：不在类型声明时使用 const 修饰符，不在函数签名的类型前使用 const 修饰符（违反 C5266 警告），能使用 constexpr 的非系统宏则使用 constexpr。
2) 缺省值：所有变量在声明时均必须指派初始值；请注意，``nullptr`` 也属于初始值。
3) 代码取舍：即使可能会十分轻微地牺牲一些性能，仍以函数为最小安全单元做必要的安全检验（例如防止恶意的内存修改导致状态不低于 Dealt 时 ``players`` 为空）；部分代码块使用更多的代码可以换取一定的性能，而使用精炼的代码则可能会牺牲一定的性能，因而允许并在性能和精炼代码之间进行平衡取舍。
...
1) 统一使用制表符进行缩进，缩进应当正确反映代码的层次结构。
2) 若一行结束时该行中存在未闭合的左圆括号、左方括号或 lambda 函数体的左花括号，则该行被记为开放行，含有开放行中最后一个左括号对应的右括号的行则被记为闭合行，随后，此处规定，开放行与闭合行之间的行的缩进量均需比开放行和闭合行的缩进量多一，开放行中开放的括号数量应当与闭合行中闭合开放行中开放的括号数量相等，必要时可在自开放行至闭合行前一行的行末处添加对齐的反斜杠符号，反斜杠符号前使用制表符进行填充（通常以行末位于最右侧的行填充一至二个制表符作为基准），但不得删除原本存在于行末的空格字符；通常，开放的括号位于开放行的行末；此规则在嵌套式相应地进行递归执行。
...
1) 为便于检查配对，花括号统一采用 Allman 风格‌，即，将左大括号置于新行并与函数签名或控制语句对齐，右大括号独占一行并与函数签名或控制语句对齐；当 lambda 函数的函数体跨行表述时，涵盖 lambda 函数体的左花括号写于行末，不遵循 Allman 风格。
2) 允许使用初始化列表且内容为空的构造函数和 lambda 函数以一行的形式呈现，前者表示函数体的一对花括号之间不应该含有任何字符，后者的花括号与函数体内容之间应保留一个空格。
3) 在 ``g++ -Wall`` 不报错误或警告且省略花括号不会变更代码语义的前提下，能不使用花括号则不使用花括号。
...
程序在识别本扑克牌类型时，会先捕获其英文，如果英文未能与本存储库所支持的任何扑克游戏匹配，则将所有数字相应地替换为中文再进行识别；以下为本存储库所支持的扑克游戏及其确定性规则和实现说明，其中所述的“牌”进一步指代有牌面值的扑克牌。
