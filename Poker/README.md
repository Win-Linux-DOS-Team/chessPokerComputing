...
1) 代码块排序：头文件 -> 宏（不包含用于类或函数内平台控制的宏） -> 命名空间（本程序未声明任何命名空间）-> 简易类型定义 -> constexpr -> 枚举类 -> 结构体（成员变量 -> 成员函数）-> 类（私有成员变量 -> 受保护成员变量 -> 公有成员变量 -> 私有成员函数 -> 受保护成员函数 -> 公有成员函数）-> 非 main 函数 -> main 函数，其中，在结构体的成员函数和类的公有成员函数中，将所有带有 ``operator`` 关键字的函数放置末尾，并依照“先重载运算符的函数后强制转型函数”的顺序排序。
...
2) 若一行结束时该行中存在未闭合的左圆括号、左方括号、lambda 函数体的左花括号或变量初始值的左花括号，则该行被记为开放行，含有开放行中最后一个左括号对应的右括号的行则被记为闭合行，随后，此处规定，开放行与闭合行之间的行的缩进量均需比开放行和闭合行的缩进量多一，开放行中开放的括号数量应当与闭合行中闭合开放行中开放的括号数量相等，必要时可在自开放行至闭合行前一行的行末处添加对齐的反斜杠符号，反斜杠符号前使用制表符进行填充（通常以行末位于最右侧的行填充一至二个制表符作为基准），但不得删除原本存在于行末的空格字符；通常，开放的括号位于开放行的行末；此规则在嵌套式相应地进行递归执行。
...
1) 为便于检查配对，花括号统一遵循 Allman 风格中花括号的风格，即，将左大括号置于新行并与函数签名或控制语句对齐，右大括号独占一行并与函数签名或控制语句对齐；当 lambda 函数的函数体跨行表述时，涵盖 lambda 函数体的左花括号写于行末，当变量初始值跨行表述时，涵盖初始值的左花括号写于行末，二者均遵循 K & R 风格中的花括号风格而不遵循 Allman 风格中的花括号风格。
